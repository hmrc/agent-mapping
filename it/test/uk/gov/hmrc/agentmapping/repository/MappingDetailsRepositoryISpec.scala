...
with DefaultPlayMongoRepositorySupport[MappingDetailsRepositoryRecord] {
  ...
  override val repository: PlayMongoRepository[MappingDetailsRepositoryRecord] =
    new MappingDetailsRepository(
      mongoComponent
    )
  ...
  "create and findByArn" should {
    ...
    "ignore creation if trying to create another record with the same arn" in {
      val newRecordSameArn = MappingDetailsRepositoryRecord(
        arn,
        Seq(MappingDetails(
          authProviderId,
          GGTag("2345"),
